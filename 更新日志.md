...
> Storyloom 自 v1.0.0 起采用语义化版本（SemVer），更早版本号（v4.x → v1.x → v2.x → v3.x → v1.5.x）为历史实验记录，
> v1.0.0 是全面重构后的正式版本，代表 Storyloom · 织叙 进入稳定阶段。

## [1.5.0] — 2026-06-21

**全面重构前端布局，借鉴洛笙创作UI设计，新增资料库/AI助手场景化/写作三栏。**

### 全局布局重构
- 顶部水平导航栏替代双层工具栏（品牌Logo + 导航Tab + 操作按钮）
- 左侧边栏改为仅在时间轴视图显示的可折叠侧边栏
- 内容区域全宽显示，信息密度更合理
- 新增 `PageHeader` / `SearchBar` / `StatsBar` / `EmptyState` 共享组件
- 所有页面遵循统一布局模式（标题+搜索+统计+内容）

### 功能增强
- **资料库（全新）**：笔记管理、文件夹树、标签系统、快速记录（Ctrl+Enter）
  - 新增 `notes` / `note_folders` / `note_tags` 数据库表
  - 新增 11 个 REST API 端点
  - 前端 `NotebookView` + `NoteEditor` + `FolderTree` 组件
- **AI助手改进**：场景化功能卡片（内容分析、灵感启发、修改建议、结构化拆分）
  - 智能对话区：上下文显示、Tab切换、工具栏
  - `AIAssistantView` + `AIFunctionCards` 组件
- **写作三栏布局（全新）**：左章节目录 + 中编辑器 + 右数据面板
  - `WritingView` 组件

### 质量
- TypeScript 编译：0 错误
- 单元测试：193 / 193 通过
- 构建产物：26.08 MB（Tauri NSIS）
- 签名：minisign v3 密钥

[v1.5.0 release →](https://github.com/YJLZSL/Storyloom/releases/tag/v1.5.0)

---

## [1.4.0] — 2026-06-21

**全新美术架构 v2.0 — 三层渲染模型 + 动画层 + 主题系统升级。**

### 全新美术架构
- **三层渲染模型**：交互层 / 内容层 / 渲染层，PixiJS v8 渐进增强背景渲染
- **动画层架构**：GSAP 引擎封装 + 10 种预设 + React Hooks（useAnimation / useStaggerAnimation / useScrollAnimation）
- **渲染层架构**：PixiBackground 组件，8 种粒子预设（织线/星空/萤火/墨滴/棱镜/落樱/气泡/极光）
- **主题系统 v2.0**：8 套完整主题配置（颜色/字体/动画/材质），运行时动态切换

### 核心动画组件
- **LoomSplash**：织机 SVG 启动动画，GSAP Timeline 4 阶段（经线落下 → 纬线穿梭 → 交织形成 → 品牌浮现）
- **PageTransition**：10 种页面过渡预设（淡入淡出/滑动/缩放/旋转/弹性弹出）
...
### 自动更新
- 新签名密钥对（minisign 格式），兼容 Tauri updater
- 自动更新配置更新（latest.json + latest.yml）
...
### 关键修复
- **自动更新/版本号**：修复 `package.json` 中 `build.publish.owner` 指向错误仓库 (`liteli1987gmail`) 的问题，改为正确的 `YJLZSL`。这是老版本无法自动更新的根本原因。
